...
*/
func (r *RegMap) Search(filelist []string) ([]Result, error) {
	...
	for level := 1; level <= r.Level; level++ {
		for _, regList := range r.Get(level) {
			...
			for _, file := range filelist {
				...
				if err != nil {
					return nil, fmt.Errorf("get content of %v error: %v", file, err)
				}
...
func SearchRegexp(directory, keyword string) ([]Result, error) {
	...
	for k, v := range resultList {
		resultList[k].Urlpath = GetUrlpath(directory, v.Urlpath)
	}
	return resultList, nil
}
